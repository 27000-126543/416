"""Fast unit tests for fixes 2, 3, 5 (no heavy model)."""
import sys
import os
sys.path.insert(0, '.')
import warnings
...
print("=" * 60)
print("Fast Verification: Fixes 2, 3, 5")
print("=" * 60)
...
print(f"  Initial: {t.usage.storage_tb:.1f}/{t.quota.storage_tb:.0f} TB")
...
assert abs(t.usage.storage_tb - 2.0) < 0.01
...
assert abs(t.usage.storage_tb - 3.0) < 0.01
print("  ✓ PASSED")
...
assert nan_count == 0, f"Expected 0 NaNs, got {nan_count}"

has_range_fail = any(f.check_name == "range_check" for f in result.failures)
has_grad_fail = any(f.check_name == "gradient_check" for f in result.failures)
print(f"  Range check failures: {has_range_fail}")
print(f"  Gradient check failures: {has_grad_fail}")
assert has_range_fail or has_grad_fail, "Expected at least range or gradient check to detect anomalies"
print("  ✓ PASSED")

print("\n" + "=" * 60)
print("ALL 3 FAST FIXES VERIFIED! ✓")
print("=" * 60)
